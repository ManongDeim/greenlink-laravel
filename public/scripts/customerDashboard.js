...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (section) {
    switch (section) {
      case "foodOrders":
        fetchAndRender("https://greenlinklolasayong.site/api/customer/food-orders", "🍴 Food Orders");
        break;
      case "farmOrders":
        fetchAndRender("https://greenlinklolasayong.site/api/customer/farm-orders", "🌾 Farm Orders");
        break;
      case "room":
        fetchAndRender("https://greenlinklolasayong.site/api/customer/room-reservations", "🏡 Room Reservations");
        break;
      case "event":
        fetchAndRender("https://greenlinklolasayong.site/api/customer/event-reservations", "📅 Event Reservations");
        break;
      default:
...
async function fetchAndRender(endpoint, title) {
  ...
  try {
    const token = localStorage.getItem("authToken"); // assuming you store Sanctum token
    ...
    console.error(err);
    content.innerHTML = `<p class="text-red-500">Failed to load data.</p>`;
  }
}

function createCard(item, type) {
  // Each type can show relevant fields
  switch (type) {
    case "🍴 Food Orders":
      return `
      <div class="p-5 bg-white rounded-xl shadow-md hover:shadow-lg transition">
        <h3 class="text-lg font-semibold text-teal-700">Food Order #${item.foodOrder_id}</h3>
        <p class="text-gray-600 mt-2">Total Bill: ₱${item.total_bill}</p>
